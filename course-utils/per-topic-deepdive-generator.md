...
3. "Visual" section: describe the single best diagram/animation for this concept (e.g. "3Blue1Brown-style vector rotation") even if you can't render it, so the learner knows what to sketch or search for.
...
7. "Review again in": 1 day / 3 days / 7 days, based on conceptual density.
</instructions>

<output_format>
Single Markdown file. Section order: Concept → Code-and-Explanation (or Worked Example) → Visual → Video → Retrieval Checkpoint → Closed-Book Rebuild → Review tag. No section may be empty — if genuinely not applicable, write "N/A — [one-sentence reason]" rather than omitting it silently.
</output_format>
