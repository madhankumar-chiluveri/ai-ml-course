...
**Floating point** — `sign × mantissa × 2^exponent` in a fixed bit budget. Mantissa bits buy precision; exponent bits buy range.

**Machine epsilon (`eps`)** — the gap between `1.0` and the next representable float. Adding less than `eps/2` to `1.0` changes nothing.

**Overflow** — a value exceeding the format's maximum, becoming `inf`. `float32` `exp` overflows above an argument of `88.7`.

**Underflow** — a value below the smallest representable magnitude, becoming `0.0`. Dangerous because `log(0)` is `-inf`, which propagates silently.

**Catastrophic cancellation** — subtracting two nearly equal numbers, destroying the significant digits that distinguished them.

**Absorption** — adding a value smaller than the running total's resolution, so the addition has no effect. The mechanism behind Demo 6.

**Shift invariance (of softmax)** — `softmax(x) == softmax(x - c)` for any constant `c`. The licence for the stability fix.

**Log-sum-exp trick** — `log(sum(exp(x))) = max(x) + log(sum(exp(x - max(x))))`. Survives overflow and underflow at identical cost.

**`log_softmax`** — computes `x - logsumexp(x)` without ever forming the probability, preserving precision the two-step form loses.

**Logits** — unnormalized scores before softmax. Losses take these rather than probabilities precisely for the Demo 4 reason.

**`float16` / `bfloat16`** — two 16-bit formats. `float16` has more mantissa and less exponent; `bfloat16` matches `float32`'s exponent range with a coarser mantissa.

**Loss scaling** — multiplying a loss by a large constant so `float16` gradients stay above underflow, then dividing it back out. Unnecessary with `bfloat16`.

**Welford's algorithm** — a one-pass variance that tracks the mean and the sum of squared deviations, never forming the large intermediates that cancel.

**Pairwise summation** — summing in a balanced tree rather than left to right, which is what `np.sum` does and why it beats a hand-written accumulator loop.
